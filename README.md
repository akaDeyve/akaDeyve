# Hey, I'm Dave 👋

Frontend developer with 2 years of professional experience, mainly building and maintaining Drupal themes for sites with a combined 100k+ organic monthly visitors.<br>
At work that's Drupal theming with Twig, SCSS, JS, plus small custom modules and forms in PHP.<br>
In my own projects it's React, React Native.

## Tech Stack

### Frontend

*Main Stack*

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![Twig](https://img.shields.io/badge/Twig-334155?style=flat&logo=twig&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![Sass](https://img.shields.io/badge/Sass-CC6699?style=flat&logo=sass&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)

*Experience with*

![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=flat&logo=react&logoColor=black)
![React Native](https://img.shields.io/badge/React_Native-61DAFB?style=flat&logo=react&logoColor=black)
![Expo](https://img.shields.io/badge/Expo-000020?style=flat&logo=expo&logoColor=white)

### Backend & Databases

*Main Stack*

![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat&logo=php&logoColor=white)
![MariaDB](https://img.shields.io/badge/MariaDB-003545?style=flat&logo=mariadb&logoColor=white)

*Experience with*

![Nodejs](https://img.shields.io/badge/Node.js-339933?style=flat&logo=node.js&logoColor=white)
![SQLite](https://img.shields.io/badge/SQLite-003B57?style=flat&logo=sqlite&logoColor=white)

### Tools & Environment

*Main Stack*

![Drupal](https://img.shields.io/badge/Drupal-0678BE?style=flat&logo=drupal&logoColor=white)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=flat&logo=vite&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)

*Experience with*

![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat&logo=figma&logoColor=white)
![Kirby CMS](https://img.shields.io/badge/Kirby_CMS-000000?style=flat)

### AI Tools

*Main Stack*

![Claude](https://img.shields.io/badge/Claude-D97757?style=flat&logo=anthropic&logoColor=white)

*Experience with*

![Mistral](https://img.shields.io/badge/Mistral-FF7000?style=flat&logo=mistralai&logoColor=white)
![Ollama](https://img.shields.io/badge/Ollama-white?style=flat&logo=ollama&logoColor=black)

## Projects

- **[Giraffe Discord Bot](https://github.com/akaDeyve/giraffe-discord-bot)** – Discord bot with AI integration and message/voice tracking stored in SQLite (Node.js, discord.js)
- **[notion-to-nextcloud](https://github.com/akaDeyve/notion-to-nextcloud)** – Self-contained Bash script that syncs a Notion workspace to Nextcloud as Markdown
- **[deepseek-bg-changer](https://github.com/akaDeyve/deepseek-bg-changer)** – Wallpaper plugin for the DeepSeek Harness web GUI (JS, CSS)

## Currently

- Working on a small web interface for my [Discord analytics bot](https://github.com/akaDeyve/giraffe-discord-bot)
- Improving my current stack skills

## Fun fact

- Cat person 🐱